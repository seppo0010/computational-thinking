...
No es razonable pensar en incorporar estos conceptos sin soporte. Y el soporte debería ser continuo, por ejemplo Google ofrece Computational Thinking for Educators[^3].

A pesar de este interés generalizado, la integración exitosa de Pensamiento Computacional en la educación obligatoria aún enfrenta problemas y desafíos sin resolver[^1][^2]. La introducción de Pensamiento Computacional en la currícula escolar en todos los niveles crea una demanda de gran escala de desarrollo profesional continuo para les docentes[^1][^3][^4][^5].

Es crucial darle al alumnado la posibilidad de trabajar en áreas que les interesen[^1].

El momento óptimo para incorporarlo es durante la adolescencia temprana[^1][^5][^6]. 

Una forma de hacerlo es sin la computadora, aprendiendo algoritmos con juegos[^1][^2].

![Nodos de entrada interconectado con una red de nodos hasta una serie de nodos de salida](inout.png)[^1]

Alternativamente existe el paradigma usar-modificar-crear

![Usar, modificar y empezar un cíclo de crear, probar, analizar, y refinar, apropiandose del resultado](usemodifycreate.png)

Al moverse por esta progresión hay que tener cuidado de no ir demasiado rápido, para no causar ansiedad, ni demasiado lento, para no producir aburrimiento.[^4]

Por otro lado existen los cursos online masivos (MOOC) que permiten llegar a más personas, pero tienen altas tasas de abandono[^1][^2].

Una forma popular de introducir cursos de programación en todos los niveles educacionales es a través de desafíos en el desarrollo de juegos. Son atractivos, orientados a metas e interactivo produciendo un aprendizaje divertido y pedagógico[^2].


### Para quiénes

El pensamiento computacional no está en sí sesgado hacia un grupo aunque la forma de enseñarlo puede estarlo. Por ejemplo si se hace a través de videojuegos puede fomentar la educación más hacia varones, mientras que si la temática es sobre historias creativas el público beneficiado sea más feminizado[^1][^7]. También hay que considerar que estas últimas pueden resultar en programas más sencillos siendo contraproducente[^7].

### Cómo aprender

Usar herramientas visuales, por sobre texto plano, puede ser más eficiente, motivar más a les estudiantes reduciendo el peso de tener que cargar con la sintaxis de un lenguaje.[^8]

#### [Logo](https://en.wikipedia.org/wiki/Logo_(programming_language))

[![](logo.png)](https://en.wikipedia.org/wiki/Logo_(programming_language))
